...
<body style="background:#f7f4ee;">
    ...
    <title>Daftar Wisata - Admin</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        body { background: #f7f4ee; }
        .container { max-width: 1100px; margin: 40px auto; background: #fff; border-radius: 18px; box-shadow: 0 2px 12px rgba(74,107,61,0.08); padding: 32px; }
        .wisata-header { font-size: 2rem; font-weight: 700; color: #2d5016; margin-bottom: 30px; }
        .btn-tambah { background: linear-gradient(135deg, #4a6b3d 0%, #5a8f4a 100%); color: #fff; border: none; padding: 12px 28px; border-radius: 18px; font-weight: 600; font-size: 1.1rem; margin-bottom: 18px; cursor: pointer; transition: background 0.2s, transform 0.3s cubic-bezier(.68,-0.55,.27,1.55), box-shadow 0.2s; box-shadow: 0 2px 8px rgba(74,107,61,0.10); }
        .btn-tambah:hover { background: #2d5016; transform: scale(1.04) translateY(-2px); box-shadow: 0 8px 24px rgba(74,107,61,0.18); }
        .table { border-radius: 18px; box-shadow: 0 2px 12px rgba(74,107,61,0.08); background: #fff; overflow: hidden; width: 100%; }
        .table th, .table td { vertical-align: middle !important; font-size: 1rem; padding: 12px 16px; }
        .badge { font-size: 0.95rem; border-radius: 12px; padding: 6px 14px; font-weight: 600; }
        .table tr { transition: background 0.2s; }
        .table tr:hover { background: #f7f4ee; }
        .aksi-btn { background: #4a6b3d; color: #fff; border: none; border-radius: 10px; padding: 8px 18px; font-weight: 600; margin-right: 6px; cursor: pointer; transition: background 0.2s, transform 0.2s; }
        .aksi-btn:hover { background: #2d5016; transform: scale(1.08); }
        .status-active { background: #4caf50; color: #fff; }
        .status-nonactive { background: #e74c3c; color: #fff; }
    </style>
</head>
<body>
    <div class="container">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span class="badge status-<?= strtolower($w['status']) ?>"><?= htmlspecialchars($w['status']) ?></span></td>
                        <td>
